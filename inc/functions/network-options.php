...
/**
 * Create Network Options panel
 *
 * Creates 'Globals Settings' panel in Network Admin
 * for defining globals paths and other Network Wide settings
 *
 * @since 1.0
 *
 */
...
function globals_network_settings() {
		...
		<div class="wrap">
			...
			<form method="post">
				<input type="hidden" name="action" value="update_globals_settings" />
					<?php wp_nonce_field('save_network_globals_settings', 'my-network-plugin'); ?>
					<table class="form-table">
						<tr valign="top">
							...
							<td>
								<input size="50" type="text" name="network_globals_settings[globals_path]"
									value="<?php
						...
						<tr valign="top">
							...
							<td>
								<input size="50"
									   type="text"
									   name="network_globals_settings[globals_google_analytics_code]"
	...
	<?php } else { ?>
		<p>Please configure WP Multisite before using these settings.  In addition, this page can only be accessed by a super admin.</p>
		<?php
		/*Note: if your plugin is meant to be used also by single wordpress
		installations you would configure the settings page here, perhaps by calling a function.*/
	}
}
