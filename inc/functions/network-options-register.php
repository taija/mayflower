 ...
 *
 * Register theme_mayflower_network_options array to hold
 * all Theme options.
 *
...
register_setting(
	// $option_group
	'theme_mayflower_network_options',
	// $option_name
	'theme_mayflower_network_options',
	// $sanitize_callback
	'mayflower_network_options_validate'
);
 ...
 */
function mayflower_network_options_validate( $input ) {
	// This is the "whitelist": current settings
	$valid_input = mayflower_network_get_options();
	// Get the array of Theme settings, by Settings Page tab
	$settingsbytab = mayflower_network_get_settings_by_tab();
 ...
 */
global $mayflower_network_tabs;
$mayflower_network_tabs = mayflower_network_get_settings_page_tabs();
/**
 ...
 */
foreach ( $mayflower_network_tabs as $tab ) {
	$tabname = $tab['name'];
	...
	foreach ( $tabsections as $section ) {
		...
		add_settings_section(
			// $sectionid
			'mayflower_network_' . $sectionname . '_section',
			// $title
			$sectiontitle,
			// $callback
			'mayflower_network_sections_callback',
			// $pageid
			'mayflower_network_' . $tabname . '_tab'
		);
	}
}
 ...
 */
function mayflower_network_sections_callback( $section_passed ) {
	global $mayflower_network_tabs;
	$mayflower_network_tabs = mayflower_network_get_settings_page_tabs();
	foreach ( $mayflower_network_tabs as $tabname => $tab ) {
		$tabsections = $tab['sections'];
		foreach ( $tabsections as $sectionname => $section ) {
			if ( 'mayflower_network_' . $sectionname . '_section' == $section_passed['id'] ) {
				?>
...
foreach ( $option_parameters as $option ) {
	...
	if ( 'custom' != $optiontype ) {
		add_settings_field(
			// $settingid
			'mayflower_network_setting_' . $optionname,
			// $title
			$optiontitle,
			// $callback
			'mayflower_network_setting_callback',
			// $pageid
			'mayflower_network_' . $optiontab . '_tab',
			// $sectionid
			'mayflower_network_' . $optionsection . '_section',
			// $args
			$option
		);
	} if ( 'custom' == $optiontype ) {
 ...
 */
function mayflower_network_setting_callback( $option ) {
	$mayflower_network_options = mayflower_network_get_options();
	$option_parameters = mayflower_network_get_option_parameters();
	$optionname = $option['name'];
	$optiontitle = $option['title'];
	$optiondescription = $option['description'];
	$fieldtype = $option['type'];
	$fieldname = 'theme_mayflower_network_options[' . $optionname . ']';

	// Output checkbox form field markup
	if ( 'checkbox' == $fieldtype ) {
		?>
		<input type="checkbox" name="<?php echo $fieldname; ?>" <?php checked( $mayflower_network_options[$optionname] ); ?> />
		<?php
	}
	// Output radio button form field markup
		...
		foreach ( $valid_options as $valid_option ) {
			?>
			<input type="radio" name="<?php echo $fieldname; ?>" <?php checked( $valid_option['name'] == $mayflower_network_options[$optionname] ); ?> value="<?php echo $valid_option['name']; ?>" />
			<span>
	...
	// Output select form field markup
		...
		foreach ( $valid_options as $valid_option ) {
			?>
			<option <?php selected( $valid_option['name'] == $mayflower_network_options[$optionname] ); ?> value="<?php echo $valid_option['name']; ?>"><?php echo $valid_option['title']; ?></option>
			<?php
		}
		?>
		</select>
	...
	// Output text input form field markup
		...
		?>
		<input type="text" name="<?php echo $fieldname; ?>" value="<?php echo wp_filter_nohtml_kses( $mayflower_network_options[$optionname] ); ?>" />
		<?php
	}
 ...
 */
function mayflower_network_setting_slider_layout() {
	$option_parameters = mayflower_network_get_option_parameters();
	$mayflower_network_slider_layouts = $option_parameters['slider_layout']['valid_options'];

	function mayflower_network_output_slider_layout( $slider_layout ) {
		$mayflower_network_options = mayflower_network_get_options();
		$slider_layout_thumbnail = mayflower_network_locate_template_uri( array( '/skins/' . $slider_layout['name'] . '.png' ), false, false );
		$current_slider_layout = ( $slider_layout['name'] == $mayflower_network_options['slider_layout'] ? true : false );
		...
		<dd><input type="radio" name="theme_mayflower_network_options[slider_layout]" <?php checked( $current_slider_layout ); ?> value="<?php echo $slider_layout['name']; ?>" /></dd>
		</dl>
	<?php
	}
	?>
<!--	<h4 style="display:block;clear:both;"><?php _e( 'White (Light)', 'mayflower' ); ?></h4> -->
	<?php
	foreach ( $mayflower_network_slider_layouts as $slider_layout ) {
			mayflower_network_output_slider_layout( $slider_layout );
	}
}
